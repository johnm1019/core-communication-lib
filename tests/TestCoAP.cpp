...
SUITE(CoAP)
{
  ...
  {
    uint8_t expected[16] = {
      0xde, 0x09, 0x45, 0x27, 0xe9, 0x18, 0xaf, 0x94,
      0x50, 0x95, 0x1f, 0x35, 0x5c, 0x72, 0xf9, 0x15 };
    unsigned char buf[16];
    memset(buf, 0, 16);
    init();
  ...
  TEST_FIXTURE(CoAPFixture, FunctionReturnBoolMatchesOpenSSL)
  {
    uint8_t expected[16] = {
      0x29, 0x3c, 0x5c, 0x0a, 0xfa, 0xad, 0x58, 0xaa,
      0x84, 0x87, 0x14, 0x1c, 0xd5, 0x8f, 0x37, 0x29 };
    unsigned char buf[16];
    memset(buf, 0, 16);
    init();
    spark_protocol.function_return(buf, 0xc3, true);
    CHECK_ARRAY_EQUAL(expected, buf, 16);
  }
  ...
  {
    uint8_t expected[16] = {
      0x1d, 0x82, 0x51, 0xa4, 0xf8, 0x9e, 0x51, 0x11,
      0x1b, 0x6f, 0x9c, 0x87, 0x0d, 0xbf, 0x21, 0x1b };
    unsigned char buf[16];
    memset(buf, 0, 16);
    init();
  ...
  {
    uint8_t expected[16] = {
      0x51, 0x96, 0xd4, 0x8b, 0x1f, 0xe2, 0x7d, 0x9b,
      0x78, 0x42, 0xf9, 0xb4, 0xe3, 0x9e, 0x09, 0x26 };
    unsigned char buf[16];
    memset(buf, 0, 16);
    init();
    spark_protocol.function_return(buf, 0xc5, 3.1415926535);
    CHECK_ARRAY_EQUAL(expected, buf, 16);
  }
